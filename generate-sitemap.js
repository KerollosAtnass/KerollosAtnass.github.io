...
// ─── دالة تنظيف النص الخارقة (تمسح الإيموجيز، الحروف المخفية، وتحول الرموز) ───
const escapeXML = (str) => {
    if (!str) return '';
    ...
    // 2. مسح حروف التحكم المخفية (Control Characters)
    cleanStr = cleanStr.replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, '');
    
    // 3. تحويل الرموز الخاصة لـ XML
    return cleanStr
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
...
async function generateSitemap() {
    ...
    certsSnap.forEach(doc => {
        ...
        uniqueImages.forEach(imgUrl => {
            if (imgUrl && typeof imgUrl === 'string') {
                // معالجة المسافات في رابط الصورة
                const cleanImgUrl = escapeXML(imgUrl).replace(/ /g, '%20');
    ...
    // سطر الوقت لضمان تغيير الملف في كل مرة يشتغل فيها الروبوت
    xml += `  \n`;
    xml += `</urlset>`;
